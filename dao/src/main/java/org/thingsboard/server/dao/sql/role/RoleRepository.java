...
public interface RoleRepository extends JpaRepository<RoleEntity, UUID> {
    ...
    @Query("SELECT r FROM RoleEntity r WHERE r.tenantId = :tenantId " +
            ...
            "OR ilike(r.description, CONCAT('%', :searchText, '%')) = true)")
    ...
    @Query("SELECT r FROM RoleEntity r WHERE r.isSystem = true OR r.tenantId IS NULL")
    List<RoleEntity> findSystemRoles();

    @Query("SELECT r FROM RoleEntity r WHERE r.tenantId = :tenantId AND r.id IN :roleIds")
    List<RoleEntity> findByTenantIdAndIdIn(@Param("tenantId") UUID tenantId,
                                           @Param("roleIds") List<UUID> roleIds);

    @Query("SELECT COUNT(u) FROM UserEntity u WHERE u.roleId = :roleId")
